...
import "./ZeekCraftToken.sol";

contract ZeekCraft is ERC721 {
    ZeekCraftToken public zeekToken;
    ...
    constructor(address _zeekTokenAddress) ERC721("ZeekCraft Recipe", "ZCR") {
        owner = msg.sender;
        zeekToken = ZeekCraftToken(_zeekTokenAddress);
    }
    ...
    function requestTokens() external {
        ...
        zeekToken.mint(msg.sender, FAUCET_AMOUNT);

        emit FaucetRequested(msg.sender, FAUCET_AMOUNT);
    }

    function withdrawTokens() external onlyOwner {
        uint256 balance = zeekToken.balanceOf(address(this));
        require(zeekToken.transfer(owner, balance), "Token transfer failed");
    }
}
